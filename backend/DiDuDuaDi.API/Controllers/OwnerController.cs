...
public class OwnerController(IOwnerRepository ownerRepository, ITranslationService translationService, ITextToSpeechService ttsService) : ControllerBase
{
    [HttpGet("dashboard")]
    ...
    public async Task<ActionResult<ApiResponse<OwnerShopDashboard>>> UpdatePoiContent([FromBody] UpdateOwnerPoiContentRequest request)
    {
        ...
        {
            string? audioUrlVi = await ttsService.GenerateAndSaveAudioAsync(request.DescriptionVi, "vi", poiId.Value.ToString());
            ...
            foreach (var lang in targetLanguages)
            {
                ...
                string audioUrl = await ttsService.GenerateAndSaveAudioAsync(translatedDesc, lang, poiId.Value.ToString());
